...
// t는 흘러가는 시간, dt는 피심의 time step 연동해서 들어감, in,out 핀 연동
{
    ...
    if (dtcnt >= Tsamp)
    {
        ...
        //MI = 0.9
        /*v_as_ref = 45 * cos(2 * M_PI * 60 * t);
        v_bs_ref = 45 * cos(2 * M_PI * 60 * t - (M_PI * 2) / 3.0);
        v_cs_ref = 45 * cos(2 * M_PI * 60 * t - (M_PI * 4) / 3.0);*/
        //MI = 1.15
        v_as_ref = 57.5 * cos(2 * M_PI * 60 * t);
        v_bs_ref = 57.5 * cos(2 * M_PI * 60 * t - (M_PI * 2) / 3.0);
        v_cs_ref = 57.5 * cos(2 * M_PI * 60 * t - (M_PI * 4) / 3.0);
        if (v_as_ref >= v_max)
        {
            v_max = v_as_ref;
            if (v_bs_ref >= v_max)
