...
// t는 흘러가는 시간, dt는 피심의 time step 연동해서 들어감, in,out 핀 연동
{
    ...
    if (dtcnt >= Tsamp)
    {
        ...
        // v_mid_ref 구하는 과정
        if (v_as_ref >= v_bs_ref) {
            if (v_bs_ref >= v_cs_ref)
            {
                v_mid_ref = v_bs_ref;
            }
            else if (v_as_ref <= v_cs_ref)
            {
                v_mid_ref = v_as_ref;
            }
            else
